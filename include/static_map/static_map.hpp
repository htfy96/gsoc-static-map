...
template <typename CharT>
using constexpr_basic_string_view = boost::basic_string_view<CharT, constexpr_char_traits<CharT>>;
...
struct EqualOp
{
    ...
    // prefer this one
    template <typename CharT>
    static constexpr bool run_impl(const constexpr_basic_string_view<CharT> &a,
                                   const constexpr_basic_string_view<CharT> &b, int)
    {
...
struct HashOp
{
    ...
    // BKDRHash.
    template <typename CharT>
    static constexpr size_t hash_impl(default_tag, const constexpr_basic_string_view<CharT> &str_view)
    {
        size_t hash_val = 0;
    ...
    static constexpr size_t hash(const T &v)
    {
        return hash_impl(std::conditional_t<std::is_integral<T>::value, integral_tag, default_tag>(), v);
    }
};


template <typename KeyT, typename MT, unsigned SIZE, typename Equal = EqualOp<KeyT>, typename Hash = HashOp<KeyT>>
class StaticMap
{
  public:
    using KeyType = KeyT;
